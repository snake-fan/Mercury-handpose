...
HandTracking::~HandTracking()
{
	...
	u_worker_thread_pool_reference(&this->pool, NULL);
                                    ...
                                    const char *models_folder)
{
	...
	try {
		...
		if (hgt->pool == nullptr) {
			throw std::runtime_error("Failed to create Mercury worker pool");
		}
		hgt->group = u_worker_group_create(hgt->pool);
		if (hgt->group == nullptr) {
			throw std::runtime_error("Failed to create Mercury worker group");
		}
		...
		HG_ERROR(hgt, "Failed to initialize Mercury workers or optimizers: %s", e.what());
		delete hgt;
		return NULL;
	}
